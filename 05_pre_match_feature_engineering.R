...
# 6. 
season_prev_map <- tibble(
  season      = c(2010, 2011, 2012, 2013, 2015, 2016, 2017, 2018, 2019),
  prev_season = c(2008, 2010, 2011, 2012, 2013, 2015, 2016, 2017, 2018)
)

all_teams <- unique(season_summary_for_history$team)

prev_season_stats <- expand_grid(team = all_teams, season_prev_map) %>%
  left_join(
    season_summary_for_history %>%
      select(
        team,
        prev_season        = season,
    ...
    by = c("team", "prev_season")
  ) %>%
  select(team, season, prev_season_win_pct,
         prev_home_win_pct, prev_away_win_pct, prev_bat_first_win_pct)
